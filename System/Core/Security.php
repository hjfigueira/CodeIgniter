...
namespace System\Core
{
	...
	class Security {
		...
		public function __construct()
		{
			...
			if (Common::config_item('csrf_protection'))
			{
				// CSRF config
				foreach (array('csrf_expire', 'csrf_token_name', 'csrf_cookie_name') as $key)
				{
					if (NULL !== ($val = Common::config_item($key)))
					{
						$this->{'_'.$key} = $val;
					}
				}
				...
				if ($cookie_prefix = Common::config_item('cookie_prefix'))
				{
			...
			$this->charset = strtoupper(Common::config_item('charset'));

			Common::log_message('info', 'Security Class Initialized');
		}
		...
		public function csrf_verify()
		{
			...
			if ($exclude_uris = Common::config_item('csrf_exclude_uris'))
			{
				$uri = Common::load_class('URI', 'core');
			...
			if (Common::config_item('csrf_regenerate'))
			{
			...
			Common::log_message('info', 'CSRF token verified');
			return $this;
		}
		...
		public function csrf_set_cookie()
		{
			...
			$secure_cookie = (bool) Common::config_item('cookie_secure');

			if ($secure_cookie && ! Common::is_https())
			{
				return FALSE;
			}
			...
			setcookie(
				...
				$this->_csrf_hash,
				$expire,
				Common::config_item('cookie_path'),
				Common::config_item('cookie_domain'),
				$secure_cookie,
				Common::config_item('cookie_httponly')
			);
			Common::log_message('info', 'CRSF cookie sent');
		...
		{
			Common::show_error('The action you have requested is not allowed.', 403);
		}
		...
		public function xss_clean($str, $is_image = FALSE)
		{
			...
			$str = Common::remove_invisible_characters($str);
		...
		public function get_random_bytes($length)
		{
			...
			if (is_readable('/dev/urandom') && ($fp = fopen('/dev/urandom', 'rb')) !== FALSE)
			{
				...
				Common::is_php('5.4') && stream_set_chunk_size($fp, $length);
